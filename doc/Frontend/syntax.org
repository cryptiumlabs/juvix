...
#+BEGIN_SRC bnf
  ...
  ;; expression type
  data-declaration ::= type <usage>? <non-capital-name> <symbols>+ : <arrow-type> = <adt>
                     | type <usage>? <non-capital-name> <symbols>+                = <adt>

  type-declaration  ::= type <usage>? <non-capital-name> <symbols>+ = <name> <type-refine>
  alias-declaration ::= type <usage>? <non-capital-name> <symbols>+ = <type-refine>
