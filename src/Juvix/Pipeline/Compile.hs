...
instance (GaloisField f) => HasBackend (BPlonk f) where
  type Ty (BPlonk f) = Plonk.PrimTy f
  type Val (BPlonk f) = Plonk.PrimVal f
  typecheck ctx = do
    let res = notImplemented -- Pipeline.contextToCore ctx Plonk.plonk
    case res of
      Right (FF.CoreDefs _order globals) -> do
        let globalDefs = HM.mapMaybe toCoreDef globals
        case HM.elems $ HM.filter isMain globalDefs of
          ...
          [IR.RawGFunction f]
            | IR.RawFunction _name usage ty (clause :| []) <- f,
              IR.RawFunClause _ [] term _ <- clause -> do
              let convGlobals = notImplemented -- map convGlobal globalDefs
                  newGlobals = notImplemented -- HM.map (unsafeEvalGlobal convGlobals) convGlobals
                  lookupGlobal = notImplemented -- IR.rawLookupFun' globalDefs
                  inlinedTerm = notImplemented -- IR.inlineAllGlobals term lookupGlobal
              (res, _) <- notImplemented -- liftIO $ exec (CorePipeline.coreToAnn inlinedTerm (IR.globalToUsage usage) ty) Param.michelson newGlobals
              case res of
                Right r -> do
                  pure r
                Left err -> notImplemented
          -- print term
          -- Feedback.fail $ show err
          somethingElse -> notImplemented
      -- Feedback.fail $ show somethingElse
      Left err -> notImplemented
  -- Feedback.fail $ "failed at ctxToCore\n" ++ show err
  typecheck _ =
  ...
  compile term = do
    -- let res = Plonk.compileTermWithWire term
    notImplemented
