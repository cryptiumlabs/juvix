...
module Juvix.Core.DepMainLang where
  ...
  iType ii g (VecElim a m mn mc k vs) =
    do  cType ii g a VStar
        let aVal = cEval a []
        cType ii g m
          (  VPi VNat (\k_ -> VPi (VVec aVal k_) (const VStar)))
        let mVal = cEval m []
        ...
        cType ii g mc
          (  VPi VNat (\ l ->
            VPi aVal (\ y ->
            VPi (VVec aVal l) (\ ys ->
            VPi (foldl vapp mVal [l, ys]) (\ _ -> foldl vapp mVal [VSucc l, VCons aVal l y ys])))))
        cType ii g k VNat
        let kVal = cEval k []
  ...
  iType ii g (EqElim a m mr x y eq) =
    do  cType ii g a VStar
        ...
        cType ii g m
          (VPi aVal (\ x_ ->
          VPi aVal (\ y_ ->
          VPi (VEq aVal x_ y_) (const VStar))))
        let mVal = cEval m []
        cType ii g mr
          (VPi aVal (\ x_ ->
